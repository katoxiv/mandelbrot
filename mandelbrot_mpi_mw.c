...
int main(int argc, char *argv[]){
  ...
  if ( myRank == 0 ){
    ...
    for (i=0; i<N_RE+1; i++){
      // Receive results and request for work
      MPI_Recv(buffer, (N_IM+1)+2, MPI_INT, MPI_ANY_SOURCE, MPI_ANY_TAG, MPI_COMM_WORLD, &status);
      
      // Unpack and store the results from the worker
      int senderRank = buffer[0];
      int completed_i = buffer[1];
      if (completed_i >= 0) { // Check for the initial case where there is no data to be sent back
        for (j = 0; j < N_IM + 1; j++) {
          nIter[completed_i][j] = buffer[j + 2];
        }
      }
      
      // Send i value to requesting process
      MPI_Send(&i, 1, MPI_INT, senderRank, 100, MPI_COMM_WORLD);
    }
    ...
    for (i=0; i<nProcs-1; i++){
      ...
      MPI_Send(&endFlag, 1, MPI_INT, senderRank, 100, MPI_COMM_WORLD);
    }
  }
  ...
  // Worker Processes
    ...
    int i = -1; // Initialize i for worker processes

    while(true){
      // Send request for work and the last calculated column (if any)
        if (i >= 0) {
          buffer[0] = myRank;
          buffer[1] = i;
          for (j = 0; j < N_IM + 1; j++) {
          ...
          buffer[0] = myRank;
          buffer[1] = -1; // Initial case: no data to send
        }
        MPI_Send(buffer, (N_IM+1)+2, MPI_INT, 0, 100+myRank, MPI_COMM_WORLD);
  ...
  if (doIO && myRank == 0) {
    ...
    write_to_file("mandelbrot_one.dat");
  }
